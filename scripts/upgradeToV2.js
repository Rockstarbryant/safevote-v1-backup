...
async function upgradeChain(chainConfig) {
  ...
  try {
    ...
    console.log("\n🧪 Data Verification:");
    console.log(`  Organizations: ${totalOrgsV1} → ${totalOrgsV2} ${totalOrgsV1.eq(totalOrgsV2) ? "✅" : "❌"}`);
    console.log(`  Polls: ${totalPollsV1} → ${totalPollsV2} ${totalPollsV1.eq(totalPollsV2) ? "✅" : "❌"}`);
...
async function main() {
  ...
  if (successful.length > 0) {
    ...
    successful.forEach((u) => {
      ...
      console.log(`    Data: Orgs ${u.dataPreserved.organizations ? "✅" : "❌"}, Polls ${u.dataPreserved.polls ? "✅" : "❌"}`);
    });
  }
